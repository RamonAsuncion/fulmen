...
namespace FulmenUI
{
    ...
    public class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            var imagePathBox = new TextBox { Width = 300, Watermark = "Image Tarball Path" };
            var browseButton = new Button { Content = "Browse" };
            var commandBox = new TextBox { Width = 300, Text = "/bin/echo Hello", Watermark = "Command" };
            ...
            browseButton.Click += async (_, __) => {
                var files = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions {
                    ...
                    AllowMultiple = false,
                    FileTypeFilter = [
                        new FilePickerFileType("Tarball") {
                            Patterns = ["*.tar", "*.tar.gz"]
                        }
                    ]
                });
                if (files != null && files.Count > 0)
                    imagePathBox.Text = files[0].Path.LocalPath;
            };
